...
## My purpose of creating this fork
  ...
  - Add some `build_<codename>.sh` to automate the building process for Xiaomi devices with Snapdragon 845
...
## Known errors of the original repository

  - On my Gentoo Linux, with `CROSS_COMPILE` and `CROSS_COMPILE_ARM32` set and `LLVM=1` though, the assembler and linker will still fall back to `/usr/bin/as` and `/usr/bin/ld` which are symbolic links that point to the respective tool targeting `x86_64-pc-linux-gnu`.
...
## Instrcutions to build
  ...
  - 3. Start to build
      ...
      - Automated approach (via `build_<codename>.sh`)
          ...
          *`beryllium` for Xiaomi POCO F1, `dipper` for Xiaomi Mi 8, `equuleus` for Xiaomi Mi 8 Pro, `perseus` for Xiaomi Mi MIX 3, `polaris` for Xiaomi Mi MIX 2S, `ursa` for Xiaomi Mi 8 Explorer Edition*
